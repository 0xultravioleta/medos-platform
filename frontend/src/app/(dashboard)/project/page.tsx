...
const SPRINTS = [
  { id: "S0", name: "Foundation", weeks: "W1-W2", dates: "Mar 1-14", totalTasks: 43, doneTasks: 43, status: "done" as const, owner_a: 24, owner_b: 17, owner_both: 2, hours: 130 },
  { id: "S1", name: "Core Data Layer", weeks: "W3-W4", dates: "Mar 15-28", totalTasks: 15, doneTasks: 15, status: "done" as const, owner_a: 8, owner_b: 6, owner_both: 1, hours: 58 },
  { id: "S2", name: "AI Clinical Docs", weeks: "W5-W6", dates: "Mar 29-Apr 11", totalTasks: 14, doneTasks: 14, status: "done" as const, owner_a: 8, owner_b: 5, owner_both: 1, hours: 58 },
  { id: "S3", name: "Revenue Cycle v1", weeks: "W7-W8", dates: "Apr 12-25", totalTasks: 12, doneTasks: 12, status: "done" as const, owner_a: 6, owner_b: 5, owner_both: 1, hours: 53 },
  { id: "S4", name: "Revenue Cycle v2", weeks: "W9-W10", dates: "Apr 26-May 9", totalTasks: 18, doneTasks: 18, status: "done" as const, owner_a: 10, owner_b: 7, owner_both: 1, hours: 76 },
  { id: "S5", name: "Pilot Prep", weeks: "W11-W12", dates: "May 10-23", totalTasks: 12, doneTasks: 12, status: "done" as const, owner_a: 6, owner_b: 5, owner_both: 1, hours: 54 },
  { id: "S6", name: "Launch", weeks: "W13", dates: "May 24-29", totalTasks: 9, doneTasks: 6, status: "in-progress" as const, owner_a: 4, owner_b: 4, owner_both: 1, hours: 33 },
  { id: "S2.5", name: "Device + Context", weeks: "W6.5", dates: "Feb 28", totalTasks: 8, doneTasks: 8, status: "done" as const, owner_a: 4, owner_b: 4, owner_both: 0, hours: 28 },
  { id: "S3F", name: "Admin Phase 1", weeks: "W8.5", dates: "Feb 28", totalTasks: 9, doneTasks: 9, status: "done" as const, owner_a: 7, owner_b: 1, owner_both: 1, hours: 20 },
];
  ...
  return (
    <div className="bg-white rounded-lg border border-[var(--medos-gray-200)] shadow-medos-sm p-3 hover:shadow-md hover:-translate-y-0.5 transition-all duration-150">
      <div className="flex items-center gap-2 mb-1.5">
        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold ${sc.bg} ${sc.text} border ${sc.border}`}>
          {task.id}
        </span>
        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${ow.bg} ${ow.text}`}>
          {ow.label}
        </span>
      </div>
      <p className="text-xs font-medium text-[var(--medos-gray-900)] leading-tight line-clamp-2 mb-2">{task.title}</p>
      <div className="flex items-center justify-between">
        <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${ec.bg} ${ec.text}`}>
...
function KanbanColumn({ title, tasks, icon: Icon, color }: { title: string; tasks: Task[]; icon: typeof Circle; color: string }) {
  ...
  return (
    <div className="flex flex-col min-h-0">
      <div className={`flex items-center gap-2 px-3 py-2.5 rounded-t-xl border border-b-0 border-[var(--medos-gray-200)] ${color}`}>
              ...
              {filtered.map((task) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${ss.bg} ${ss.text} border ${ss.border}`}>
                        {ss.label}
                      </span>
                    </td>
            ...
            return (
              <div key={sprint.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className={`h-7 rounded-lg ${trackColor} overflow-hidden`} style={{ width: `${barWidth}%` }}>
                    <div className={`h-full ${statusColor} rounded-lg transition-all duration-500`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
            ...
            {MILESTONES.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-start gap-4 relative">
                  <div className={`relative z-10 flex items-center justify-center w-8 h-8 rounded-lg ${isDone ? "bg-emerald-100" : "bg-gray-100"}`}>
                    <Diamond className={`w-4 h-4 ${isDone ? "text-emerald-600" : "text-gray-400"}`} />
                  </div>
                  <div className="flex-1 pt-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium ${isDone ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-gray-50 text-gray-600 border border-gray-200"}`}>
                        {isDone ? "Done" : "Pending"}
                      </span>
                    </div>
            ...
            return (
              <div key={sprint.id} className="flex items-center gap-3">
                <div className="w-10 flex-shrink-0">
                  <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold ${sc.bg} ${sc.text}`}>
                    {sprint.id}
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="h-6 bg-[var(--medos-gray-100)] rounded-md overflow-hidden">
                    <div
                      className={`h-full rounded-md flex items-center px-2 ${sprint.status === "done" ? "bg-emerald-400" : "bg-amber-400"}`}
                      style={{ width: `${barPct}%` }}
                    >
            ...
            return (
              <div key={epic.id} className="flex items-center gap-3">
                <div className="w-16 flex-shrink-0">
                  <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold ${ec.bg} ${ec.text}`}>
              ...
              {SPRINTS.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center gap-2 text-[10px]">
                    <span className={`inline-flex items-center px-1 py-0.5 rounded font-bold ${sc.bg} ${sc.text} w-8 justify-center`}>
                      {s.id}
                    </span>
